...
//! [![Publish
//! packages](https://github.com/anweiss/cddl/workflows/Publish%20packages/badge.svg?branch=0.5.4&event=release)](https://github.com/anweiss/cddl/actions?query=workflow%3A%22Publish+packages%22)
//! [![Build and
...
//! | any                  | any valid JSON                |
//!
//! Since JSON objects only support keys whose types are JSON strings, member
...
//! Unquoted member keys used with the double arrow syntax must resolve to one
//! of the supported data types that can be used to validate JSON strings
//! (`text` or `tstr`). Occurrence indicators can be used to validate key/value
//! pairs in a JSON object and the number of elements in a JSON array; depending
//! on how the indicators are defined in a CDDL data definition. CDDL groups,
//! generics, sockets/plugs and group-to-choice enumerations are all parsed and
...
//! ## Validating CBOR
//!
//! > Incomplete. Under development. Less complete than JSON validation
//! > functions.
//!
...
//! where allocation is required. Allocation is also used for error handling and
//! diagnostics.
//!
