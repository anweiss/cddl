...
fn test_type_check_with_size_constraint() -> Result<(), Box<dyn Error>> {
  ...
  let cbor_bytes = Value::Map(vec![(
    Value::Text("digitalSourceType".to_string()),
  ...
  if let Err(e) = result_without_size {
    ...
    assert!(
      e.to_string().contains("expected type tstr, got Bytes"),
  ...
  assert!(
    result_with_size.is_err(),
    "Validation should fail when bytes are provided instead of text, even with size constraint"
  );

  if let Err(e) = result_with_size {
    println!("Expected error (with size): {}", e);
    assert!(
      e.to_string().contains("expected type tstr"),
      "Error message should indicate type mismatch"
    );
  }
