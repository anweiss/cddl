...
fn validate_cbor_start_with_tagged_array() -> Result<(), Box<dyn Error>> {
  ...
  let cose_sign1_array = Value::Array(vec![
    headers_map.clone(),
    payload_bstr.clone(),
    signature_bstr.clone(),
  ]);
  ...
  let cose_sign1_array_nil_payload = Value::Array(vec![
    headers_map.clone(),
    payload_nil,
    signature_bstr.clone(),
  ]);
  ...
  ciborium::ser::into_writer(
    &valid_cbor_nil_payload_val,
    &mut valid_cbor_nil_payload_bytes,
  )?;
  ...
  let wrong_array_payload_val = Value::Array(vec![
    headers_map.clone(),
    wrong_payload_type,
    signature_bstr.clone(),
  ]);
