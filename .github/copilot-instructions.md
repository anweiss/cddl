...
The version in `Cargo.toml` is the **single source of truth** for the crate version. The following files contain WASM package versions that **must** be kept in sync with `Cargo.toml` whenever the version is bumped:

1. `pkg/package.json` — main WASM build output (auto-generated by `wasm-pack`, but verify after build)
2. `cddl-languageserver/package.json` — published language server WASM package
3. `cddl-lsp/server/pkg/package.json` — LSP extension's bundled WASM package
4. `www/package.json` — website / playground package

When bumping the version:

1. Update the `version` field in `Cargo.toml`
2. Update the `"version"` field in **all four** `package.json` files listed above to match
3. Verify no version drift by running: `grep -E '"version"' pkg/package.json cddl-languageserver/package.json cddl-lsp/server/pkg/package.json www/package.json`
